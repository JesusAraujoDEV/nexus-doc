...
nexus-doc is a single-page web frontend built with Vite, React 18 and TypeScript, using shadcn/ui (Radix primitives) for its component layer and TanStack Query for server-state. Routing is client-side via react-router-dom; styling is Tailwind CSS. Package name: `vite_react_shadcn_ts`.

Full specification: [`docs/spec.md`](docs/spec.md) *(to be written)*.
...
Locked decisions (no re-discuss): Vite (not Next/CRA); shadcn/ui as the component source (own the code, no external UI kit); TanStack Query for server-state (no Redux/global store for data fetching); Tailwind for styling; `@/*` import alias maps to `src/*`.

## Architecture rules

- **Layering**: Pages (`src/pages`) → feature components (`src/components/*`) → shared UI primitives (`src/components/ui`); data access through TanStack Query hooks in `src/hooks`; cross-cutting helpers in `src/lib`.
- **No business logic in views.** Components render props/state and dispatch events; server-state lives in TanStack Query, not in component-local state.
- **shadcn/ui is owned code, not a dependency.** Primitives in `src/components/ui` are generated by shadcn (`components.json`); extend or wrap them, but keep the generated files editable rather than forking patterns elsewhere.
- **Data fetching goes through TanStack Query** — no ad-hoc `fetch`/`useEffect` data loading in components; query/mutation logic lives in hooks.
- **Routing is centralized** via react-router-dom; validate form and boundary input with Zod schemas + react-hook-form resolvers.
- **Imports use the `@/` alias** (`@/*` → `src/*`); prefer it over deep relative paths.
...
src/
  ...
  components/         # feature components
    ...
    ui/               # shadcn/ui primitives (Radix) — generated, owned code
    NavLink.tsx
...
```
Requires Node >= 20. A `bun.lockb` is present, so `bun` may also be used.
